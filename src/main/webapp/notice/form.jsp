...
<c:set var="action" value="/web/notice/write" />

<c:if test="${not empty param['idx'] }">
	<c:set var="btnTitle" value ="공지사항 수정" />
	<c:set var="action" value="/web/notice/update" />
</c:if>
		...
		<%-- 수정할 페이지아니라면  --%>
				<form action="${action }" method="POST">
					...
					<div id="title_wrapper">
						<label>제목&nbsp;&nbsp;&nbsp;&nbsp;: <input type="text" name="title" value="${noticeInfo.title }"></label>
					</div>
					
					<div id="contents_wrapper">
						<label>내용<br><textarea name="contents" cols="100" rows="10">${noticeInfo.contents }</textarea></label>
					</div>
